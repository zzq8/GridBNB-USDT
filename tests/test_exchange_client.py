...
class TestExchangeClientInit:
    ...
    @patch('exchange_client.settings')
    @patch('exchange_client.ccxt.binance')
...
class TestMarketData:
    ...
    def mock_client(self):
        """创建 mock 的交易所客户端"""
        with patch('exchange_client.settings') as mock_settings:
            mock_settings.BINANCE_API_KEY = 'test_key'
            mock_settings.BINANCE_API_SECRET = 'test_secret'
            mock_settings.DEBUG_MODE = False
            mock_settings.SAVINGS_PRECISIONS = {'DEFAULT': 2}

            with patch('exchange_client.ccxt.binance'):
                client = ExchangeClient()
...
class TestBalance:
    ...
    async def test_fetch_funding_balance_disabled(self, mock_client):
        """测试理财功能关闭时的行为"""
        with patch('exchange_client.settings.ENABLE_SAVINGS_FUNCTION', False):
